...
class DocumentTest(utils.IntegrationTestCase):
    ...
    O = Document
    default_ctor_kwargs = {u'_id': u'123'}
    json = {u'id': u'123',
            u'title': u'Contract',
            u'ctime': u'2014-10-29T15:40:20Z',
            u'mtime': u'2014-10-29T15:40:20Z',
            u'lang': u'sv'}

    def test_id(self):
        serialized_values = self._value_pairs([u'123', u'456'])

        d = self.o()
        self.assertFalse(d.id is None)
        ...
        self._test_invalid_field('id77')

        # test that for every pair (v1, v2) in serialized_values
        # json[u'id']=v2; O._from_json_obj(json).id == v1
        for value_in, value_out in serialized_values:
            json = dict(self.json)
            json[u'id'] = value_out
            d = self.O._from_json_obj(json)
            self.assertTrue(isinstance(d, self.O))
            self.assertEqual(d.id, value_in)
        ...
        for _, value in serialized_values:
            ...
            json[u'id'] = value
            d = self.O._from_json_obj(json)
            json2 = d._to_json_obj()
            self.assertEqual(value, json2[u'id'])
            self.assertEqual(type(value), type(json2[u'id']))

    def test_title(self):
        unicode_err = u'title must be unicode text, or ascii-only bytestring'
        self._test_attr(
            attr_name='title',
            good_values=[u'foo', (b'bar', u'bar'), u'ą'],
            bad_type_values=[([], u'unicode or str')],
            bad_val_values=[(u'ą'.encode('utf-8'), unicode_err),
                            (u'', u'title must be non-empty string')],
            serialized_name='title',
            serialized_values=[u'foo', u'ą'])

    def test_language(self):
        variant_err = r".*could be Language's variant name.*"
        good_values = [('english', Language.english),
                       ...
                       ('islandic', Language.islandic),
                       ('lithuanian', Language.lithuanian),
                       ('latvian', Language.latvian)]
        serialized_values = [(Language.english, u'en'),
                             ...
                             (Language.norwegian, u'no'),
                             (Language.greek, u'el'),
                             (Language.finnish, u'fi'),
                             (Language.estonian, u'et'),
                             (Language.islandic, u'is'),
                             (Language.lithuanian, u'lt'),
                             (Language.latvian, u'lv')]
        self._test_attr(
            attr_name='language',
            good_values=good_values + map(lambda x: x[1], good_values),
            bad_type_values=[({}, u'Language'), (0, u'Language')],
    ...
    def test_creation_time(self):
        ...
        self._test_server_attr(attr_name='creation_time',
                               serialized_name=u'ctime',
                               serialized_values=[time_tuple])

    def test_modification_time(self):
        time_tuple = (datetime(2014, 10, 29, 15, 40, 20, tzinfo=tzutc()),
                      u'2014-10-29T15:40:20Z')
        self._test_server_attr(attr_name='modification_time',
